...
def process_tables(markdown):
    ...
    def format_cell(cell):
        cell = replace_markdown_formatting(remove_links(clean(cell)))
        if len(cell) > MAX_WIDTH:
            return cell[:MAX_WIDTH - 3] + ''
        return cell

    output = ['.TS', 'allbox tab(|);', 'c' * len(headers) + '.']
    output.append('|'.join([format_cell(h) for h in headers]))
